...
public class CameraScreenNail extends SurfaceTextureScreenNail {
	...
	public boolean mStatusOfFaceDetection = true;   //for facedetection open and close by fuqiang

    public interface Listener {
        void requestRender();
        // Preview has been copied to a texture.
        void onPreviewTextureCopied();
		void startFaceDetectionInCamera();   //for facedetection open and close by fuqiang
		void stopFaceDetectionInGallery();
    }
    ...
    public void draw(GLCanvas canvas, int x, int y, int width, int height) {
        synchronized (mLock) {
			...
			if((x < 0)&&(y < 0))   //for facedetection open and close by fuqiang
			{
				if(mStatusOfFaceDetection == true)
				{
					mListener.stopFaceDetectionInGallery();
					mStatusOfFaceDetection = false;
				}
			}
			else
			{
				if(mStatusOfFaceDetection == false)
				{
					mListener.startFaceDetectionInCamera();
					mStatusOfFaceDetection = true;
				}
			}
